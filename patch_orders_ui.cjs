...
c = c.replace(
    /\/\/ Check for pending photos([\s\S]*?)const issues = getOrderIssues\(order\);/,
    `// Check for pending photos$1const issues = getOrderIssues(order);
                               const isOtherExpanded = expandedOrderId && expandedOrderId !== order.id;`
);

// We need to minimize other orders view.
c = c.replace(
    /<div className="flex-1 min-w-0">\s*<div className="flex items-center gap-2">\s*<h3 className="font-bold text-base md:text-lg text-slate-800 dark:text-white truncate">\{order\.title\}<\/h3>([\s\S]*?)<\/div>\s*<div className="flex items-center gap-2 text-xs md:text-sm text-slate-500 dark:text-slate-400 mt-0\.5 md:mt-1 overflow-x-auto whitespace-nowrap">([\s\S]*?)<\/div>\s*<\/div>\s*<\/div>\s*<div className="flex items-center gap-3 ml-14 md:ml-0">/,
    `<div className="flex-1 min-w-0">
                                                       <div className="flex items-center gap-2">
                                                           <h3 className="font-bold text-base md:text-lg text-slate-800 dark:text-white truncate">{order.title}</h3>$1</div>
                                                       {!isOtherExpanded ? (
                                                            ...
                                                            <div className="text-xs text-slate-400 mt-0.5 whitespace-nowrap overflow-hidden text-ellipsis">{new Date(order.dateCreated).toLocaleDateString()}</div>
                                                       )}
                                                   </div>
                                               </div>
...
c = c.replace(
    /\{isExpanded \? <ChevronUp className="w-5 h-5 text-slate-400"\/> : <ChevronDown className="w-5 h-5 text-slate-400"\/>\}\s*<\/div>\s*<\/div>\s*<\/div>\s*\{isExpanded && \(/,
    `</>)}
