...
import { ShoppingCart, CheckCircle, Activity, PlusCircle, ShoppingBag, ArrowDownCircle, AlertTriangle, Search, Clock, FileText } from 'lucide-react';
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {canCreateWarehouse && (
                ...
                {userRole === UserRole.TECHNICAL ? 'Criar Requisição' : 'Criar Pedido ao Armazém'}
                </button>
            )}
      ...
      {(showReceipts || showShortages || showSearch) && (
                ...
                <div className="flex flex-col gap-1 mt-1">
                    ...
                    {hasPendingApproval && (
                        <div className="flex items-center gap-1 text-xs font-bold text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/30 px-2 py-1 rounded-full w-fit">
                            ...
                            Há requisições por aprovar
                        </div>
                    )}
                </div>
            </StatCard>
        ) : (
            // Placeholder for users who can create but not see list? Or just hide?
            // Hiding for clean UI if they have no permission
            null
        )}
