...
// 2658-2684 replacement
const targetRegex = /onClick=\{\(e\) => \{[\s\S]*?\}\}\s*className=\`p-3 text-center rounded-full flex-shrink-0 mr-3 cursor-help hover:scale-110 active:scale-95 transition-all outline-none \$\{[\s\S]*?\}\`>\s*\{type === 'OPEN' \? \([\s\S]*?\)\s*\):\s*\([\s\S]*?\)\}/;
c = c.replace(targetRegex, `onClick={(e) => {
                                                           e.stopPropagation();
                                                           if (pickingCode === 'C') {
